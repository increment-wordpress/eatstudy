...
/**
 * Add wp-parsi-date admin page settings
 * */
function wpp_add_settings_menu()
{
    ...
    if ($wpp_settings['submenu_move'] != 'disable') {
        add_submenu_page(
            'options-general.php',
            __('Parsi Settings', 'wp-parsi-date'),
            __('Parsi Settings', 'wp-parsi-date'),
            'manage_options',
            'wp-parsi-settings',
            'wpp_render_settings'
...
{
    $tabs = array(
        'core' => sprintf(__('%s Core', 'wp-parsi-date'), '<span class="dashicons dashicons-admin-site"></span>'),
        'conv' => sprintf(__('%s Converts', 'wp-parsi-date'), '<span class="dashicons dashicons-admin-settings"></span>'),
        'plugins' => sprintf(__('%s Plugins compability', 'wp-parsi-date'), '<span class="dashicons dashicons-admin-plugins"></span>')
    );
...
{
    $options = array(
        'enable' => __('Enable', 'wp-parsi-date'),
        'disable' => __('Disable', 'wp-parsi-date')
    );
    $settings = apply_filters('wpp_registered_settings', array(
        'core' => apply_filters('wpp_core_settings', array(
            'admin_lang' => array(
                'id' => 'admin_lang',
                'name' => __('Change Locale in admin', 'wp-parsi-date'),
                'type' => 'radio',
                'options' => $options,
                'std' => 'disable',
                'desc' => __('This option change WordPress locale to Persian in Admin', 'wp-parsi-date')
            ),
            'user_lang' => array(
                'id' => 'user_lang',
                'name' => __('Change Locale in theme', 'wp-parsi-date'),
                'type' => 'radio',
                'options' => $options,
                'std' => 'disable',
                'desc' => __('This option change WordPress locale to Persian in theme', 'wp-parsi-date')
            ),
            'persian_date' => array(
                'id' => 'persian_date',
                'name' => __('Shamsi date', 'wp-parsi-date'),
                'type' => 'radio',
                'options' => $options,
                'std' => 'disable',
                'desc' => __('By enabling this, Dates will convert to Shamsi (Jalali) dates', 'wp-parsi-date')
            ),
            'submenu_move' => array(
                'id' => 'submenu_move',
                'name' => __('Move page to submenu?', 'wp-parsi-date'),
                'type' => 'radio',
                'options' => $options,
                'std' => 'disable',
                'desc' => __('With enabling this option, page item will be moved to Settings menu as submenu.', 'wp-parsi-date')
            ),
        )),
        'conv' => apply_filters('wpp_conv_settings', array(
            'conv_nums' => array(
                'id' => 'conv_nums',
                'name' => __('Persian digits', 'wp-parsi-date'),
                'type' => 'header'
            ),
            'conv_page_title' => array(
                'id' => 'conv_page_title',
                'name' => __('Page title', 'wp-parsi-date'),
                'type' => 'radio',
                'options' => $options,
                'std' => 'disable'
            ),
            'conv_title' => array(
                'id' => 'conv_title',
                'name' => __('Post title', 'wp-parsi-date'),
                'type' => 'radio',
                'options' => $options,
                'std' => 'disable'
            ),
            'conv_contents' => array(
                'id' => 'conv_contents',
                'name' => __('Post content', 'wp-parsi-date'),
                'type' => 'radio',
                'options' => $options,
                'std' => 'enable'
            ),
            'conv_excerpt' => array(
                'id' => 'conv_excerpt',
                'name' => __('Post excerpt', 'wp-parsi-date'),
                'type' => 'radio',
                'options' => $options,
                'std' => 'disable'
            ),
            'conv_comments' => array(
                'id' => 'conv_comments',
                'name' => __('Comments text', 'wp-parsi-date'),
                'type' => 'radio',
                'options' => $options,
                'std' => 'disable'
            ),
            'conv_comment_count' => array(
                ...
                'name' => __('Comments count', 'wp-parsi-date'),
                'type' => 'radio',
                'options' => $options,
                'std' => 'disable'
            ),
            ...
            'conv_cats' => array(
                'id' => 'conv_cats',
                'name' => __('Categories', 'wp-parsi-date'),
                'type' => 'radio',
                'options' => $options,
                'std' => 'disable'
            ),
            ...
            'conv_arabic' => array(
                'id' => 'conv_arabic',
                'name' => __('Fix arabic characters', 'wp-parsi-date'),
                'type' => 'radio',
                'options' => $options,
                'std' => 'disable',
                'desc' => __('Fixes arabic characters caused by wrong keyboard layouts', 'wp-parsi-date')
            ),
            'conv_permalinks' => array(
                'id' => 'conv_permalinks',
                'name' => __('Fix permalinks dates', 'wp-parsi-date'),
                'type' => 'radio',
                'options' => $options,
                'std' => 'disable',
                'desc' => __('By enabling this, dates in permalinks converted to Shamsi (Jalali) date', 'wp-parsi-date')
            ),
            'sep_font' => array(
                'id' => 'sep_font',
                'type' => 'header'
...
function wpp_render_settings()
{
    ...
    <div class="wrap wpp-settings-wrap">
        <h2><?php _e('Parsi Settings', 'wp-parsi-date') ?></h2>
