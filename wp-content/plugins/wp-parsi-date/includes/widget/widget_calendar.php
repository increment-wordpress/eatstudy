...
class parsidate_calendar extends WP_Widget
{
    public function __construct()
    {
        ...
        if (version_compare($wp_version, '4.3', '>=')) {
            parent::__construct(false, __('Jalali Date Calender', 'wp-parsi-date'), 'description=' . __('Jalali Date Calender', 'wp-parsi-date'));
        } else {
            parent::WP_Widget(false, __('Jalali Date Calender', 'wp-parsi-date'), 'description=' . __('Jalali Date Calender', 'wp-parsi-date'));
        }
    }
    ...
    public function form($instance)
    {
        ...
        <p style="text-align:right; direction:rtl">
            <label><?php _e('Title:', 'wp-parsi-date') ?></label>
            <input style="width: 200px;" id="<?php echo $this->get_field_id('parsidate_calendar_title'); ?>"
                   ...
                   value="<?php echo(empty($instance['parsidate_calendar_title']) ? __('Jalali Date Calender', 'wp-parsi-date') : $instance['parsidate_calendar_title']) ?>"/>
        </p>
        <?php
