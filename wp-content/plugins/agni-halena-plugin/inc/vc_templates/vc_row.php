 ...
 */
$output = $after_output = $fluid = $design_css = $row_bg_css = $row_bg_overlay = $fullheight = $flexrow = $columnsplacement = $equalheight = $row_css = $row_css_classes = $fallback_bg = $overflow_visible = $disable_element = $bg_parallax_class = '';
...
if( $fullwidth == 'has-fullwidth-column' || $fullwidth == 'has-fullwidth-column-no-padding' ){
	$fluid = '-fluid';
}
...
if ( 'yes' === $disable_element ) {
	if ( vc_is_page_editable() ) {
		$disable_element = 'vc_hidden-lg vc_hidden-xs vc_hidden-sm vc_hidden-md';
	} else {
		return '';
	}
}

if( $atts['overflow_visible'] == '1' ){
	$overflow_visible = 'agni_row_overflow_visible';
	$row_css = (!empty($atts['z-index']))?' z-index: '.$atts['z-index'].'; ':'';
}
...
$design_css_array_0 = !empty($design_css_array[0])?$design_css_array[0]:'';
if( !empty($design_css_array_0) || !empty($row_css) ){
	//$custom_heading_css = !empty($design_css_array[0])?$design_css_array[0].$custom_heading_css:$custom_heading_css
	//$design_css_array_0 = !empty($design_css_array[0])?$design_css_array[0]:'';
	$design_css .= ' style="';
	$design_css .= $design_css_array_0;
	$design_css .= $row_css.'" data-css-default="'.$design_css_array_0.$row_css.'"';
